...
/* ── Multer: accept CSV/JSON uploads up to 2 GB ─────────────────── */
const upload = multer({
  dest: UPLOAD_DIR,
...
/* ── Helper: safe col list from query string ─────────────────────── */
...
/* ── Helper: build WHERE clause from filter conditions ───────────── */
...
/* ═══════════════════════════════════════════════════════════════════
   POST /api/mx/upload
   Accepts: multipart CSV file OR JSON body { headers, rows }
   Returns: { sessionId, rowCount, colCount, headers, colTypes }
   ═══════════════════════════════════════════════════════════════════ */
router.post("/mx/upload", upload.single("file"), async (req: Request, res: Response) => {
  try {
    ...
    if (req.file) {
      ...
      /* ── JSON body { headers: string[], rows: object[] } ── */
      ...
      /* Write temp JSON and import via DuckDB */
    ...
    /* Use DESCRIBE which gives column_name + column_type reliably across DuckDB versions */
...
/* ═══════════════════════════════════════════════════════════════════
   GET /api/mx/rows?session=<id>&from=0&limit=100&cols=col1,col2
   ═══════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════
   GET /api/mx/sort?session=<id>&col=X&dir=asc&from=0&limit=100&cols=...
   ═══════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════
   POST /api/mx/filter
   Body: { session, conditions: [{col, op, value}], searchQ, from, limit, cols }
   ═══════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════
   PATCH /api/mx/cell
   Body: { session, row: N (0-indexed), col: "colName", value: "newValue" }
   Strategy: read full parquet → update row → write back
   (for massive files this would use a delta log; good enough for now)
   ═══════════════════════════════════════════════════════════════════ */
router.patch("/mx/cell", async (req: Request, res: Response) => {
  ...
  try {
    ...
    s.sortCache.clear(); // invalidate sort caches
    res.json({ ok: true });
  } catch (err) {
...
/* ═══════════════════════════════════════════════════════════════════
   DELETE /api/mx/session?session=<id>
   ═══════════════════════════════════════════════════════════════════ */
