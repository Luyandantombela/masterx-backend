...
export function createSession(fields: Omit<SessionMeta, "sessionId" | "createdAt" | "lastUsed" | "sortCache">): SessionMeta {
  const session: SessionMeta = {
    ...fields, sessionId: uuidv4(), createdAt: Date.now(), lastUsed: Date.now(), sortCache: new Map(),
  };
...
export function deleteSession(id: string): void {
  const s = sessions.get(id);
  if (s) { try { fs.unlinkSync(s.parquetPath); } catch {} sessions.delete(id); }
}

setInterval(() => {
  const now = Date.now();
  for (const [id, s] of sessions) {
    if (now - s.lastUsed > SESSION_TTL_MS) deleteSession(id);
  }
}, 10 * 60 * 1000);
